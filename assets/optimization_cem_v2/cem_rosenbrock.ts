// Version d3.js des figures assets/optimization_cem/cem_rosenbrock_*.svg :
// exécution du CEM sur la fonction de Rosenbrock (contours de f, ellipses de la
// proposal distribution, échantillons et élites par itération).
//
// Les données (échantillons + statut élite, mu/sigma par itération) viennent de
// cem_rosenbrock_data.json (extrait des SVG matplotlib d'origine par
// cem_rosenbrock_extract.py) ; l'apparence (fonction objectif, niveaux de
// contours, couleurs) de cem_rosenbrock_config.json. Les deux sont éditables à
// la main.
//
...
//          (sans distribution) ; fit = + distribution ajustée (rouge, = proposal
//          de l'itération k+1).
import * as d3 from 'd3';
import config from './cem_rosenbrock_config.json';
import data from './cem_rosenbrock_data.json';
...
// ---------------------------------------------------------------------------
// Contours de la fonction objectif : calculés une seule fois, partagés par
// toutes les figures de la page.
// ---------------------------------------------------------------------------
...
function setupFigure(el: SVGSVGElement): (frame: Frame | null) => void {
	...
	return (frame: Frame | null): void => {
		...
		const iter = data.iterations[frame.k - 1];
		const next = data.iterations[frame.k]; // proposal ajustée = celle de k+1
		...
		} else if (frame.phase === 'fit' && next) {
			distributions.push({ ...next.proposal, color: config.proposal.fittedColor });
		}
